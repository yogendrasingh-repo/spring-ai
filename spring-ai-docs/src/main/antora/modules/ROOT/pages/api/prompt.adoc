...
The `ChatConnector` class, for instance, is analogous to the core JDBC library in the JDK.
...
It is common to use the `call` method of `ChatConnector` that takes a `Prompt` instance and returns an `ChatResponse`.
