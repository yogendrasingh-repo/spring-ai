...
The `stream` method takes a `Prompt` request similar to `ChatConnector` but it streams the responses using the reactive Flux API.
...
The `ChatConnector` and `StreamingChatClient` implementations are provided for the following Model providers:
