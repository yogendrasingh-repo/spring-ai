...
import org.springframework.ai.bedrock.anthropic3.BedrockAnthropic3ChatConnector;
	...
	public void chatCompletionDisabled() {
		...
		new ApplicationContextRunner()
			...
			.run(context -> {
				...
				assertThat(context.getBeansOfType(BedrockAnthropic3ChatConnector.class)).isEmpty();
			});
		...
		new ApplicationContextRunner().withPropertyValues("spring.ai.bedrock.anthropic3.chat.enabled=true")
			...
			.run(context -> {
				...
				assertThat(context.getBeansOfType(BedrockAnthropic3ChatConnector.class)).isNotEmpty();
			});
